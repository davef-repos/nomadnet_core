...
class UIBackend:
    ...
    A UI backend implementation (urwid-based TUI, Neovim plugin, etc.)
    receives lifecycle callbacks from the NomadNetworkApp core.
    """

    def on_exit(self, app):
        """Called during NomadNetworkApp shutdown. Perform UI cleanup here."""
        pass

    def on_message_received(self, app):
        """Called when a new message arrives via LXMF delivery."""
        pass

    def get_glyph(self, name):
        """Return a UI glyph/symbol by name. May be None."""
        return None

    def schedule_redraw(self, app, delay=0.0):
        """Request a UI redraw after an optional delay."""
        pass
...
class NomadNetworkApp:
    ...
    def __init__(self, configdir = None, rnsconfigdir = None, daemon = False, force_console = False, ui_backend = None):
        ...
        self.uimode        = None
        self._ui_backend   = ui_backend if ui_backend is not None else UIBackend()
        ...
        from nomadnet_core.core.Directory import Directory as NomadDirectory
        self.directory = NomadDirectory(self)

        from nomadnet_core.core.RRC import RRCManager
        ...
        if self.enable_node:
            ...
            from nomadnet_core.core.Node import Node as NomadNode
            self.node = NomadNode(self)
        else:
            self.node = None
            if os.path.isfile(self.pnannouncedpath):
                try:
        ...
        from nomadnet_core.core.Conversation import Conversation as NomadConversation
        from nomadnet_core.core.Directory import Directory as NomadDirectory
        RNS.Transport.register_announce_handler(NomadConversation)
        RNS.Transport.register_announce_handler(NomadDirectory)
        ...
        RNS.log("Nomad Network Core initialized", RNS.LOG_VERBOSE)
    ...
    def lxmf_delivery(self, message):
        ...
        from nomadnet_core.core.Conversation import Conversation as NomadConversation
        NomadConversation.ingest(message, self)
    ...
    def conversations(self):
        from nomadnet_core.core.Conversation import Conversation as NomadConversation
        return NomadConversation.conversation_list(self)

    def has_unread_conversations(self):
        from nomadnet_core.core.Conversation import Conversation as NomadConversation
        if len(NomadConversation.unread_conversations) > 0:
            return True
        else:
            return False
    ...
    def conversation_is_unread(self, source_hash):
        ...
        if src_bytes in NomadConversation.unread_conversations:
            return True
        if src_bytes in NomadConversation.failed_conversations:
            return True
        return False
    ...
    def notify_message_recieved(self):
        self._ui_backend.on_message_received(self)
        ...
        if "client" in self.config:
            for option in self.config["client"]:
                ...
                if option == "user_interface":
                    value = value.lower()
                    self.uimode = value
                    if value == "text" and "textui" in self.config:
                        # Store textui config as plain values (no nomadnet.ui constant refs)
                        ...
                        if not "colormode" in self.config["textui"]:
                            self.config["textui"]["colormode"] = "16"
                        else:
                            colormode = self.config["textui"]["colormode"].lower()
                            if colormode in ("monochrome", "16", "88", "256", "24bit"):
                                self.config["textui"]["colormode"] = colormode
                            else:
                                raise ValueError("The selected Text UI color mode is invalid")

                        if not "theme" in self.config["textui"]:
                            self.config["textui"]["theme"] = "dark"
                        else:
                            theme = self.config["textui"]["theme"].lower()
